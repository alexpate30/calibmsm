...
Finally, metadata is added and the appropriate \code{class} is defined so that the data is in the same format as the output from \code{calib_blr}, meaning it can be used with the S3 generic \code{plot}. Note that in this example `assessed.transitions` = `valid.transitions`, but this may not be the case if you only estimated calibration curves for a subset of the possible transitions.
