...
This is followed by fitting a cox model to predict probability of being censored. This below code chunk is what happens internally in `calib_blr` if one sets `w_covs = c("year", "agecl", "proph", "match")`.

```{r}
model_ipcw <- survival::coxph(survival::Surv(dtcens, dtcens_s) ~ year + agecl + proph + match, data = ebmtcal)
model_ipcw
```
...
```{r}
kmfit <- survival::survfit(survival::Surv(dtcens, dtcens_s) ~ year, data = ebmtcal)
```
