...
data.raw <- dplyr::inner_join(ebmtcal, tps0 %>% subset(j == 1), by = dplyr::join_by("id"))
...
for (state in 1:6){
  ...
  for (group in 1:3){
    data.raw.list.sort.temp[[group]] <- dplyr::arrange(data.raw.list[[group]], !!rlang::sym(paste("pstate", state, sep = "")))
  }
...
pv.comb <- dplyr::inner_join(pv.out.groups[[1]], pv.out.groups[[2]], by = dplyr::join_by("id")) %>%
  dplyr::inner_join(pv.out.groups[[3]], by = dplyr::join_by("id")) %>%
  dplyr::inner_join(pv.out.groups[[4]], by = dplyr::join_by("id")) %>%
  dplyr::inner_join(pv.out.groups[[5]], by = dplyr::join_by("id")) %>%
  dplyr::inner_join(pv.out.groups[[6]], by = dplyr::join_by("id"))
