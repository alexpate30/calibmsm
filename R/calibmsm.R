...
calib_msm <- function(data_ms,
  ...
  ### Obtain vector of states which each individual is in at time t

  ### Get vector of patient id's
  v1 <- data_raw$id

  ### Initialise state_poly as an empty list with one slot per individual.
  ### Each slot will be filled with state that individual is in at time t.
  state_poly <- vector("list", length(v1))

  ### Loop over valid transitions only (small number of states). For each state k,
  ### we find which individuals are in it using a vectorised %in% check, then
  ### append k to those individuals' slots in state_poly directly.
  ### This avoids ever materialising the n x max_state matrix in memory.
  for (k in valid_transitions){
    ### Get list of individuals in state k
    ids_k <- extract_ids_states(data_ms, tmat, k, t)
    ### Find the position of individuals in state k in the vector
    positions <- which(v1 %in% ids_k)
    ### Assign this to the state_poly vector
    state_poly[positions] <- k
  }

  ### Replace any NULL slots (individuals not found in any valid transition state)
  ### with NA. Without this, unlist() would silently drop them, producing a vector
  ### shorter than n and causing a length mismatch in the mutate below.
  state_poly[sapply(state_poly, is.null)] <- NA
...
#'
#' @noRd
calib_msm_old <- function(data_ms,
                      data_raw,
                      j,
                      s,
                      t,
