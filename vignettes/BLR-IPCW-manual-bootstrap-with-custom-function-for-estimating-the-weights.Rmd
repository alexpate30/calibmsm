...
We now remind ourselves of the procedure for generating a confidence interval, and estimate a confidence interval for a BLR-IPCW calibration curve using the internal bootstrapping procedure, as in done in the main overview vignette.

* Item 1 Resample validation dataset with replacement
* Item 2 Landmark the dataset for assessment of calibration
* Item 3 Calculate inverse probability of censoring weights
* Item 4 Fit the preferred calibration model in the landmarked dataset (restricted cubic splines or loess smoother)
* Item 5 Generate observed event probabilities for a fixed vector of predicted transition probabilities (specifically the predicted transition probabilities from the non-bootstrapped landmark validation dataset)
...
The observed event probabilities can then be estimated using `calc_calib_blr` and a call to the `weights` argument.
...
Finally, metadata is added and the appropriate \code{class} is defined so that the data is in the same format as the output from \code{calc_calib_blr}, meaning it can be used with the S3 generic \code{plot}. Note that in this example `assessed.transitions` = `valid.transitions`, but this may not be the case if you only estimated calibration curves for a subset of the possible transitions.
