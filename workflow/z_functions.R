...
convert.mstate.DGM1.nocens <- function(cohort.in, max.follow){
  ...
  #   covs <- c("x12", "x13", "x15", "x24", "x25", "x34", "x35", "x45")
...
###
### 1.2) Convert data into a format that can be analysed using the mstate package
...
###
### 1.2B) Convert a dataset into mstate format, but when the censoring mechanism has already been generated, and is a variable within cohort.in
###
...
calc.calib.blr.ipcw.boot.se <- function(data.mstate, data.raw, t.eval, p.est, n.boot, max.weight = 10){
  ...
  get_int_slope_boot <- function(data, indices){
    ...
    output.obj <- c(intercepts, intercepts.mspec, intercepts.pspec,
                    #intercepts.DGMspec,
                    slopes, slopes.mspec, slopes.pspec,
                    #slopes.DGMspec,
                    diff.pred.obs, diff.pred.obs.mspec, diff.pred.obs.pspec)
  ...
  colnames(boot.obj$t) <- c(paste("int", 1:5, sep = ""), paste("int.mspec", 1:5, sep = ""),
                            paste("int.pspec", 1:5, sep = ""),
                            #paste("int.DGMspec", 1:5, sep = ""),
                            paste("slope", 1:5, sep = ""), paste("slope.mspec", 1:5, sep = ""),
                            paste("slope.pspec", 1:5, sep = ""),
                            #paste("slope.DGMspec", 1:5, sep = ""),
                            paste("diff.pred.obs", 1:5, sep = ""), paste("diff.pred.obs.mspec", 1:5, sep = ""),
                            paste("diff.pred.obs.pspec", 1:5, sep = ""))
  #, paste("diff.pred.obs.DGMspec", 1:5, sep = ""))
...
calc.weights <- function(data.raw, t.eval, max.weight = 10){
  ...
  cens.model.pspec <- coxph(Surv(model.cens.times, model.cens.s) ~ x12 + x13 + x15 + x24 + x25 + x34 + x35 + x45, data = data.raw)
  ...
  lp.pspec <- as.matrix(data.raw[, c("x12", "x13", "x15", "x24", "x25", "x34", "x35", "x45")]) %*% as.numeric(coefficients(cens.model.pspec))
...
###
### Create plots for data estimated with BLR-IPCW and PV
###
plot_calib_msm <- function(x, ..., combine = TRUE, ncol = NULL, nrow = NULL,
  ...
  #     bottom = NULL
  #     left = NULL
  #     axis.titles.text.x = paste("PRedicted risk")
  #     axis.titles.text.y = paste("Perfectly calibrated \n Observed risk")
...
###
### Create plots for data estimated using MLR-IPCW
###
...
#
# test1 <- gridExtra::arrangeGrob(grobs = plots.list,
#                                 layout_matrix = base::matrix(base::seq_len(nrow*ncol),
#                                                              nrow = nrow,
#                                                              ncol = ncol,
#                                                              byrow = TRUE),
#                                 top = NULL, bottom = "Predicted risk", left = "Observed risk")
#
#
# test2 <-
#
#
# png("figures/aaa_TEST2.png", width = 7, height = 7, unit = "in", res = 300)
# grid::grid.draw(test1)
# dev.off()
#
#
# png("figures/aaa_TEST3.png", width = 7, height = 7, unit = "in", res = 300)
# grid::grid.draw(test2)
# dev.off()
#
#
# library(ggplot2)
# library(patchwork)
# plot(test)
# test <- ggpubr::get_legend(plots.list[[1]])
#
# plots.list[[1]] + plots.list[[2]] + patchwork::plot_layout(ncol = 2, nrow = 1)
