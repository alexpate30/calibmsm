...
### Don't actually need to do this, because already done it in the supplementary material, however can add it here at a later date

### The conclusion, is that calibration directly into state 5 is good, however calibration into state 3 is not great.
### This should actually motivate going deeper into net risks/cause-specific hazards here...
...
###
### Calculate risks out of state 1
...
for (j in 1:4){
  ...
  for (r1.count in 1:length(r1.range)){
    ...
    for (r2.count in 1:length(r2.range)){
      tp_pred[[j]][[r1.count]][[r2.count]] <- data.frame(matrix(NA, ncol = 16, nrow = nrow(ebmt)))
...
### Create tp ###
#################
save.image("workflow/supp_diagnosis_miscalibration_prep_data.RData")
